  ...
  return (
    <div className="profile-page-container container">
            ...
            <div key={index} className="card game-history-item mb-3">
              <div className="card-body">
                <h3 className="card-title">
                  Game {Math.ceil(indexOfLastGame/3)} - Total Score: {game.score}
                </h3>
                ...
                <img
                  ...
                  src={game.image_path}
                  alt="Game"
                  className="game-image img-thumbnail"
                />
                <br />
                <br />
                <p className={`caption-selected ${game.score === 0 ? 'incorrect' : ''}`}>
                  Caption Selected: {game.caption}
                </p>
                {game.score === 0 && (
                  <p id="correct_selected">Correct Caption: {game.rightcaption}</p>
                )}
              </div>
            </div>
          ))
      ...
      <div className="action-buttons text-center">
        <button className="btn btn-primary m-2" onClick={handlePlayGame}>
          Play Game
        </button>
        <button className="btn btn-secondary m-2" onClick={handleLogoutClick}>
          Logout
        </button>
        <button className="btn btn-primary m-2" onClick={fetchLeaderboardData}>
          Leaderboard
        </button>
        ...
        {showLeaderboard && (
          <div className="leaderboard-table">
            ...
            <button className="btn btn-danger m-2" onClick={() => setShowLeaderboard(false)}>
              Cancel
            </button>
          </div>
        )}
